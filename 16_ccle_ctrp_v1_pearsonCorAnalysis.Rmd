---
title: "ctrpv1 ccle pearson correlations"
...
#list.files()
expr <- fread(file = paste0(cellSamples2, "_222.CellLines_13432.Genes_",
                            "RMA.Normalized_GeneExpression.txt"),
       sep = "\t", header = T, quote = "")

#expr <- fread(file = paste0(cellSamples2, "_821.CellLines_18762.Genes_",
#                            "RMA.Normalized_GeneExpression.txt"),
#       sep = "\t", header = T, quote = "")
...
gsva.H <- fread(file = paste0(cellSamples1, "_222.CellLines_1.PreTx.everSamples",
...
#Read in auc (knn imputation) data from GDSC1
auc.knn <- fread(file = paste0(cellSamples1, 
                               "_222.CellLines_338.Drugs_AUC_knnImputation.txt"),
                 sep = "\t", header = T, quote = "")
...
any(gsva.data.all$CellLine %in% colnames(auc.knn)[2:ncol(auc.knn)])
...
knn.data1 <- auc.knn %>% 
  as.data.frame() %>% 
  pivot_longer(!DRUG_NAME, names_to = "CellLine", values_to = "AUC") %>% 
  unique()
...
druglist <- fread(file = paste0("v10.M1.informer_set.txt"),
                  sep = "\t", header = T, quote = F)

druglist1 <- druglist %>% 
  rename("DRUG_NAME" = "cpd_name",
         "DrugTarget" = "gene_symbol_of_protein_target",
         "DrugActivity" = "target_or_activity_of_compound") %>% 
  mutate(DRUG_NAME = str_replace_all(DRUG_NAME, 
                                     pattern = " ", replacement = "_")) %>% 
  filter(DRUG_NAME %in% unique(knn.data1$DRUG_NAME)) %>% 
  select(DRUG_NAME, DrugTarget, DrugActivity)


#Read in cell line info
setwd(paste0(dir, dir1, dir2, "/predictiveModelFrameWork_files/"))
cl.tissue <- fread(file = paste0(cellSamples1, "_", cellSamples2, 
                                 "_222.CellLines_tissueType.txt"),
                  sep = "\t", header = T, quote = F)

cl.tissue <- cl.tissue %>% 
  rename("CellLine_orig" = "CellLine",
         "CellLine" = "CellLine1") %>% 
  filter(CellLine %in% clines)
head(cl.tissue)
...
expr_auc.pre <- gsva.data.all %>% 
  ...
  left_join(druglist1, by = "DRUG_NAME") %>% 
  #filter(!Targets == "Unclassified") %>% 
  select(AUC, CellLine, CellLine_orig, tissue,
         Gene_Set, Regulator, Tx_timePoint, 
         ssGSEA_Score, DRUG_NAME, everything())

head(expr_auc.pre)
dim(expr_auc.pre)

length(unique(expr_auc.pre$CellLine))
length(unique(expr_auc.pre$DRUG_NAME))
...
expr_auc.pre <- expr_auc.pre %>% 
  ...
  ungroup() %>% 
  select(AUC, ssGSEA_Score, Gene_Set, CellLine, CellLine_orig, tissue,
         DRUG_NAME, DrugTarget, DrugActivity, AUC.scaled, everything())
...
#Combine
tmp.np <- c(tmp.n, tmp.p, "selumetinib", "sirolimus")


d.tmp <- expr_auc.pre %>% 
  select(DRUG_NAME, DrugTarget) %>% 
  unique()

#Get data for mek inhibitors
drug.keep <-  c("selumetinib", "sirolimus")
...
p1 <- corTest_df1 %>% 
  ...
  facet_wrap(~Gene_Set) +
  geom_point(mapping = aes(color = DRUG_NAME), size = 0.4, show.legend = F) +
  geom_text_repel(mapping = aes(label = ifelse(label == "TRUE", DRUG_NAME, ""),
                                color = DRUG_NAME
                                ),
                  size = 2, fontface = "bold", max.overlaps = 30, 
                  show.legend = F) +
...
mek.cor <- corTest_df1 %>% 
  ...
  mutate(Pearson_cor = round(Pearson_cor, 3),
         ...
         pval2 = format.pval(pvalue, digits = 3, eps = 2e-16, nsmall = 3))
...
for (i in drug.keep) {
  ...
  #annotate(geom = "text", x = 1.29, y = 1.05, size = 3, fontface = "bold",
    annotate(geom = "text", x = .6, y = 1.05, size = 3, fontface = "bold",
           label = paste0("R = ", as.numeric(mek.cor$Pearson_cor), 
                          #", FWER = ", mek.cor$fwer2))
                          #", FDR = ", mek.cor$fdr2),
                           ", pvalue = ", mek.cor$pval2)) 
...
#Read in ssGSEA scores for preTX MR regulon Signatures
gsva.H <- fread(file = paste0(cellSamples1, "_222.CellLines_1.PostTx.everSamples",
                              ".SMARCD3.Signatures_ssGSEA.Scores.txt"),  
                              #".SMARCD3.Signatures_ssGSEA.Scores.txt"),
       sep = "\t", header = T, quote = "")
