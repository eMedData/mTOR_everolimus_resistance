...
AQUAPORIN.MEDIATED_TRANSPORT_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

AQUAPORIN.MEDIATED_TRANSPORT_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

AQUAPORIN.MEDIATED_TRANSPORT <- c(AQUAPORIN.MEDIATED_TRANSPORT_R, 
                  AQUAPORIN.MEDIATED_TRANSPORT_S) %>% unique()


#AXON_GUIDANCE
keep <- c("AXON_GUIDANCE")
AXON_GUIDANCE_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

AXON_GUIDANCE_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

AXON_GUIDANCE <- c(AXON_GUIDANCE_R, 
                  AXON_GUIDANCE_S) %>% unique()
...
#CELL_JUNCTION
keep <- c("CELL_JUNCTION")
CELL_JUNCTION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

CELL_JUNCTION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

CELL_JUNCTION <- c(CELL_JUNCTION_R, 
                  CELL_JUNCTION_S) %>% unique()


#DNA_REPAIR
keep <- c("DNA_REPAIR")
DNA_REPAIR_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

DNA_REPAIR_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

DNA_REPAIR <- c(DNA_REPAIR_R, 
                  DNA_REPAIR_S) %>% unique()

#ECM_ORGANIZATION
keep <- c("ECM_ORGANIZATION")
ECM_ORGANIZATION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ECM_ORGANIZATION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ECM_ORGANIZATION <- c(ECM_ORGANIZATION_R, 
                  ECM_ORGANIZATION_S) %>% unique()


#ER_GOLGI_TRANSPORT
keep <- c("ER_GOLGI_TRANSPORT")
ER_GOLGI_TRANSPORT_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ER_GOLGI_TRANSPORT_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ER_GOLGI_TRANSPORT <- c(ER_GOLGI_TRANSPORT_R, 
                  ER_GOLGI_TRANSPORT_S) %>% unique()
...
not.keep <- c("BIOCARTA_AT1R_PATHWAY", 
              ...
              paste0("REACTOME_JNK_C_JUN_KINASES_PHOSPHORYLATION_AND_",
                     "ACTIVATION_MEDIATED_BY_ACTIVATED_HUMAN_TAK1"))
top1.4_R <- stateR2 %>% 
  filter(!Gene_Set %in% not.keep) %>% 
  filter(metaphenotype == keep & 
                metaphenotype_2 == keep.notes,
...
not.keep <- c("REACTOME_SPRY_REGULATION_OF_FGF_SIGNALING")
top1.6_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep & 
                metaphenotype_2 == keep.notes) %>%
  dplyr::arrange(FDR) %>%
...
#Merge all GF
top1 <- c(top1.2, top1.3, top1.4, top1.5, top1.6) %>% na.omit()
...
not.keep <- c("KEGG_MELANOGENESIS", "HYPERTROPHIC_CARDIOMYOPATHY_HCM")
top12_R <- stateR2 %>% 
  filter(!Gene_Set %in% not.keep) %>% 
  filter(metaphenotype == keep) %>%
...
top12 <- c(top12_R, top12_S) %>% unique() 


#ORGANELLE_BIOGENESIS
keep <- c("ORGANELLE_BIOGENESIS")
ORGANELLE_BIOGENESIS_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ORGANELLE_BIOGENESIS_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

ORGANELLE_BIOGENESIS <- c(ORGANELLE_BIOGENESIS_R, 
                  ORGANELLE_BIOGENESIS_S) %>% unique()


#POST.TRANSLATION_MODIFICATION
keep <- c("POST.TRANSLATION_MODIFICATION")
POST.TRANSLATION_MODIFICATION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

POST.TRANSLATION_MODIFICATION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

POST.TRANSLATION_MODIFICATION <- c(POST.TRANSLATION_MODIFICATION_R, 
                  POST.TRANSLATION_MODIFICATION_S) %>% unique()


#PROTEIN_LOCALIZATION
keep <- c("PROTEIN_LOCALIZATION")
PROTEIN_LOCALIZATION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

PROTEIN_LOCALIZATION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

PROTEIN_LOCALIZATION <- c(PROTEIN_LOCALIZATION_R, 
                  PROTEIN_LOCALIZATION_S) %>% unique()
...
#RNA_PROCESSING
keep <- c("RNA_PROCESSING")
RNA_PROCESSING_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

RNA_PROCESSING_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

RNA_PROCESSING <-c(RNA_PROCESSING_R, RNA_PROCESSING_S) %>% unique()


#SENSOTRY_PERCEPTION
keep <- c("SENSORY_PERCEPTION")
SENSORY_PERCEPTION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SENSORY_PERCEPTION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SENSORY_PERCEPTION <-c(SENSORY_PERCEPTION_R, SENSORY_PERCEPTION_S) %>% unique()


#SLC.MEDIATED_TRANSPORT
keep <- c("SLC.MEDIATED_TRANSPORT")
SLC.MEDIATED_TRANSPORT_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SLC.MEDIATED_TRANSPORT_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SLC.MEDIATED_TRANSPORT <-c(SLC.MEDIATED_TRANSPORT_R, SLC.MEDIATED_TRANSPORT_S) %>% unique()
...
keep <- c("SYNAPTIC_TRANSMISSION")
SYNAPTIC_TRANSMISSION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SYNAPTIC_TRANSMISSION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

SYNAPTIC_TRANSMISSION <-c(SYNAPTIC_TRANSMISSION_R, SYNAPTIC_TRANSMISSION_S) %>% unique()
...
TRANSCRIPTION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSCRIPTION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSCRIPTION <-c(TRANSCRIPTION_R, TRANSCRIPTION_S) %>% unique()



#TRANSFERRIN.MEDIATED_TRANSPORT
keep <- c("TRANSFERRIN.MEDIATED_TRANSPORT")
TRANSFERRIN.MEDIATED_TRANSPORT_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSFERRIN.MEDIATED_TRANSPORT_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSFERRIN.MEDIATED_TRANSPORT <-c(TRANSFERRIN.MEDIATED_TRANSPORT_R, 
                                   TRANSFERRIN.MEDIATED_TRANSPORT_S) %>% unique()


#TRANSLATION
keep <- c("TRANSLATION")
TRANSLATION_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSLATION_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

TRANSLATION <-c(TRANSLATION_R, TRANSLATION_S) %>% unique()



#VESICLE.MEDIATED.MEDIATED_TRANSPORT
keep <- c("VESICLE.MEDIATED_TRANSPORT")
VESICLE.MEDIATED_TRANSPORT_R <- stateR2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

VESICLE.MEDIATED_TRANSPORT_S <- stateS2 %>% 
  dplyr::filter(metaphenotype == keep) %>%
  pull(Gene_Set) %>%
  as.character()

VESICLE.MEDIATED_TRANSPORT <-c(VESICLE.MEDIATED_TRANSPORT_R, 
                                   VESICLE.MEDIATED_TRANSPORT_S) %>% unique()
...
ord <- c(GF_SIGNALING, IMMUNE_RESPONSE, GPCR_SIGNALING, TRANSCRIPTION,
         TP53_SIGNALING, SENSORY_PERCEPTION, ANTI.APOPTOSIS,
         TRANSFERRIN.MEDIATED_TRANSPORT, SYNAPTIC_TRANSMISSION, PROTEIN_LOCALIZATION,
         AQUAPORIN.MEDIATED_TRANSPORT, VESICLE.MEDIATED_TRANSPORT,
         SLC.MEDIATED_TRANSPORT, ECM_ORGANIZATION, TF_ACTIVITY, 
         
         
         CELL_CYCLE, METABOLISM, RNA_PROCESSING, RHO_GTPASE_SIGNALING,
         ER_GOLGI_TRANSPORT, POST.TRANSLATION_MODIFICATION, 
         ORGANELLE_BIOGENESIS,DNA_REPAIR, APOPTOSIS, TRANSLATION, 
         CELL_JUNCTION, STRESS_RESPONSE, AXON_GUIDANCE)
...
AXON_GUIDANCE_1 <- rownames(heat2) %in% AXON_GUIDANCE
CELL_CYCLE_1 <- rownames(heat2) %in% CELL_CYCLE
CELL_JUNCTION_1 <- rownames(heat2) %in% CELL_JUNCTION
DNA_REPAIR_1 <- rownames(heat2) %in% DNA_REPAIR
ECM_ORGANIZATION_1 <- rownames(heat2) %in% ECM_ORGANIZATION
ER_GOLGI_TRANSPORT_1 <- rownames(heat2) %in% ER_GOLGI_TRANSPORT
...
ORGANELLE_BIOGENESIS_1 <- rownames(heat2) %in% ORGANELLE_BIOGENESIS
POST.TRANSLATION_MODIFICATION_1 <- rownames(heat2) %in% POST.TRANSLATION_MODIFICATION
PROTEIN_LOCALIZATION_1 <- rownames(heat2) %in% PROTEIN_LOCALIZATION
RHO_GTPASE_SIGNALING_1 <- rownames(heat2) %in% RHO_GTPASE_SIGNALING
RNA_PROCESSING_1 <- rownames(heat2) %in% RNA_PROCESSING
SENSORY_PERCEPTION_1 <- rownames(heat2) %in% SENSORY_PERCEPTION
SLC.MEDIATED_TRANSPORT_1 <- rownames(heat2) %in% SLC.MEDIATED_TRANSPORT
STRESS_RESPONSE_1 <- rownames(heat2) %in% STRESS_RESPONSE
SYNAPTIC_TRANSMISSION_1 <- rownames(heat2) %in% SYNAPTIC_TRANSMISSION
...
TRANSFERRIN.MEDIATED_TRANSPORT_1 <- rownames(heat2) %in% TRANSFERRIN.MEDIATED_TRANSPORT
TRANSLATION_1 <- rownames(heat2) %in% TRANSLATION
VESICLE.MEDIATED_TRANSPORT_1 <- rownames(heat2) %in% VESICLE.MEDIATED_TRANSPORT



#Merge all gene set names that will be labeled
top.merg <- c(top1, top2, top3, 
              #top4, 
              top5, 
              #top6, 
              top7, 
...
tmp.3 <- tmp. %>%
  ...
  dplyr::mutate(color = ifelse(metaphenotype == "APOPTOSIS", "#0570b0",
...
hmapGSEA <- Heatmap(heat2,
        ...
        row_split = c(rep(1, each = length(GF_SIGNALING)), 
                      ...
                      rep(4, each = length(TRANSCRIPTION)), 
                      rep(5, each = length(TP53_SIGNALING)),
                      rep(6, each = length(SENSORY_PERCEPTION)),
                      rep(7, each = length(ANTI.APOPTOSIS)),
                      rep(8, each = length(TRANSFERRIN.MEDIATED_TRANSPORT)),
                      rep(9, each = length(SYNAPTIC_TRANSMISSION)),
                      rep(10, each = length(PROTEIN_LOCALIZATION)),
                      rep(11, each = length(AQUAPORIN.MEDIATED_TRANSPORT)),
                      rep(12, each = length(VESICLE.MEDIATED_TRANSPORT)),
                      rep(13, each = length(SLC.MEDIATED_TRANSPORT)),
                      rep(14, each = length(ECM_ORGANIZATION)),
                      rep(15, each = length(TF_ACTIVITY)),
                      rep(16, each = length(CELL_CYCLE)),
                      rep(17, each = length(METABOLISM)),
                      rep(18, each = length(RNA_PROCESSING)),
                      rep(19, each = length(RHO_GTPASE_SIGNALING)),
                      rep(20, each = length(ER_GOLGI_TRANSPORT)),
                      rep(21, each = length(POST.TRANSLATION_MODIFICATION)),
                      rep(22, each = length(ORGANELLE_BIOGENESIS)),
                      rep(23, each = length(DNA_REPAIR)),
                      rep(24, each = length(APOPTOSIS)),
                      rep(25, each = length(TRANSLATION)),
                      rep(26, each = length(CELL_JUNCTION)),
                      rep(27, each = length(STRESS_RESPONSE)),
                      rep(28, each = length(AXON_GUIDANCE))
                      ),
        row_gap = unit(2, "mm"),
        cluster_rows = F,
  ...
  Heatmap(TRANSCRIPTION_1 + 0,
          name = "TRANSCRIPTION", col = c("0" = "white", "1" = "#FFFF00"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#FFFF00",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(TP53_SIGNALING_1 + 0,
          ...
          column_names_gp = gpar(fontsize = fsize, col = "#CD1076",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(SENSORY_PERCEPTION_1 + 0,
          name = "SENSORY_PERCEPTION", col = c("0" = "white", "1" = "#FCD3BC"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#FCD3BC",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(ANTI.APOPTOSIS_1 + 0, 
          ...
          column_names_gp = gpar(fontsize = fsize, col = "#4A1486",
                                 fontface = "bold"), column_names_rot = 30) +
    Heatmap(TRANSFERRIN.MEDIATED_TRANSPORT_1 + 0, 
          name = "TRANSFERRIN.MEDIATED_TRANSPORT", col = c("0" = "white", "1" = "#DC7C00"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#DC7C00",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(SYNAPTIC_TRANSMISSION_1 + 0, 
          name = "SYNAPTIC_TRANSMISSION", col = c("0" = "white", "1" = "#00CC7C"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#00CC7C",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(PROTEIN_LOCALIZATION_1 + 0, 
          name = "PROTEIN_LOCALIZATION", col = c("0" = "white", "1" = "#75B33A"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#75B33A",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(AQUAPORIN.MEDIATED_TRANSPORT_1 + 0, 
          ...
          column_names_gp = gpar(fontsize = fsize, col = "#006400",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(VESICLE.MEDIATED_TRANSPORT_1 + 0, 
          name = "VESICLE.MEDIATED_TRANSPORT", col = c("0" = "white", "1" = "#85BEBD"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#85BEBD",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(SLC.MEDIATED_TRANSPORT_1 + 0, 
          name = "SLC.MEDIATED_TRANSPORT", col = c("0" = "white", "1" = "#6CC5CC"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#6CC5CC",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(ECM_ORGANIZATION_1 + 0, 
          name = "ECM_ORGANIZATION", col = c("0" = "white", "1" = "#C99546"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#C99546",
                                 fontface = "bold"), column_names_rot = 30) +
  ...
  Heatmap(METABOLISM_1 + 0,
          ...
          column_names_gp = gpar(fontsize = fsize, col = "#FFB6C1",
                                 fontface = "bold"), column_names_rot = 30) +
    Heatmap(RNA_PROCESSING_1 + 0,
          name = "RNA_PROCESSING", col = c("0" = "white", "1" = "#EEE8CD"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#EEE8CD",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(RHO_GTPASE_SIGNALING_1 + 0,
          ...
          column_names_gp = gpar(fontsize = fsize, col = "orange",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(ER_GOLGI_TRANSPORT_1 + 0,
          name = "ER_GOLGI_TRANSPORT", col = c("0" = "white", "1" = "#99DA00"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#99DA00",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(POST.TRANSLATION_MODIFICATION_1 + 0,
          name = "POST.TRANSLATION_MODIFICATION", col = c("0" = "white", "1" = "#ABC083"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#ABC083",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(ORGANELLE_BIOGENESIS_1 + 0,
          name = "ORGANELLE_BIOGENESIS", col = c("0" = "white", "1" = "#E3767B"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#E3767B",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(DNA_REPAIR_1 + 0,
          name = "DNA_REPAIR", col = c("0" = "white", "1" = "#B6534E"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#B6534E",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(APOPTOSIS_1 + 0, 
          ...
          column_names_gp = gpar(fontsize = fsize, col = "#0570b0",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(TRANSLATION_1 + 0, 
          name = "TRANSLATION", col = c("0" = "white", "1" = "#FEC857"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#FEC857",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(CELL_JUNCTION_1 + 0, 
          name = "CELL_JUNCTION", col = c("0" = "white", "1" = "#D66621"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#D66621",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(STRESS_RESPONSE_1 + 0,
          name = "STRESS_RESPONSE", col = c("0" = "white", "1" = "#8093E3"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#8093E3",
                                 fontface = "bold"), column_names_rot = 30) +
  Heatmap(AXON_GUIDANCE_1 + 0,
          name = "AXON_GUIDANCE", col = c("0" = "white", "1" = "#9DFFFF"),
          show_heatmap_legend = F, width = unit(4, "mm"),
          column_names_gp = gpar(fontsize = fsize, col = "#9DFFFF",
                                 fontface = "bold"), column_names_rot = 30)

#HEAT MAP WITHOUT ROW NAMES
...
meta.stack <- tmp5 %>% 
  ...
  mutate(label = ifelse(metaphenotype_2 == "ESTROGEN_SIGNALING","ESTROGEN_SIGNALING",
                 ...
                 ifelse(metaphenotype_2 == "RAS_SIGNALING","RAS_SIGNALING",
...
tiff(file = paste(prefix, cellSamples, "ssGSEA", rows, term, comp,
                  ...
                  sep = "_"), width = 20, height = 12, units = "in", res = 300, 
     compression = "lzw")
...
tiff(file = paste(prefix, cellSamples, "ssGSEA", rows, term, comp,
                  ...
                  sep = "_"), width = 10, height = 8, units = "in", res = 300)
